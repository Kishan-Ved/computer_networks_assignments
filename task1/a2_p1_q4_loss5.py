...
PCAP_FILE_SERVER = "p1_q4_loss5_server_scalable.pcap"

class MyTopo(Topo):
    def build(self, losspercent):
        # Add switches
        ...
        self.addLink(s2, s3, bw=50, loss=losspercent)
...
def parse_args():
    ...
    parser.add_argument('--loss', type=int, default=5,
                        help='Loss percent on the link between s2 and s3')
...
def runTopology(option, ccs, losspercent):
    topo = MyTopo(losspercent)
...
if __name__ == '__main__':
    setLogLevel('info')
    args = parse_args()
    runTopology(args.option, args.ccs, args.loss)
